...
uint32_t current_flash_led_mode = 0;


//ZTE_CAM_LJ_20120409 add yuv sensor flash function
#if defined(CONFIG_MACH_AURORA) || defined(CONFIG_MACH_DANA)||defined(CONFIG_MACH_HAYES)||defined(CONFIG_MACH_ILIAMNA)
...
extern uint16_t current_preview_gain_flash;

int zte_get_flash_auto_flag_value(void)
{
	uint16_t ov5640_auto_flash_threshold = 0xF0;
	pr_err("adsfg g_preview_gain_flash=%d\n",current_preview_gain_flash);
	if( current_preview_gain_flash >= ov5640_auto_flash_threshold){
	  	return 1;
	}else{
	  	return 0;
	}
...
int32_t msm_camera_flash_set_led_state(
	...
	{
	int32_t rc=0;
	pr_err("%s led_state=%d  current_flash_led_mode =%d \n",__func__,led_state,current_flash_led_mode);
	switch (led_state) {
	case MSM_CAMERA_LED_RELEASE:
	case MSM_CAMERA_LED_OFF:

#if defined CONFIG_ISPCAM
	rc = isp_camera_flash_led_disable();
#elif defined CONFIG_FLSH_ADP1650 		
	rc = ADP1650_disable();
#else
	...
	}else if((current_flash_led_mode ==1)&&(zte_get_flash_auto_flag_value()== 1)){
			rc = msm_camera_flash_led_enable();
	}
#endif
