...
#define TAOS_INT_GPIO (34)
*/
#define TAOS_INT_GPIO 49
...
static u16 gain_trim_param = 190;
...
///////////////end//////////////////////////////////////////////////
		...
		case TAOS_IOCTL_PROX_CALIBRATE:
			...
			do{			
				...
				printk(KERN_ERR "taos:prox_pulse_cnt_param=%d s(%d) hi(%d) c(%d)\n",prox_pulse,sat_prox,taos_cfgp->prox_threshold_hi,count);
				if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x0E), prox_pulse))) < 0) {
                          	printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed\n");
			...
			#if defined (CONFIG_MACH_METIS) || defined(CONFIG_MACH_NEX)
			if (taos_cfgp->prox_threshold_lo < ((sat_prox*25)/100)) {				
				taos_cfgp->prox_threshold_hi = ((sat_prox*30)/100);
				taos_cfgp->prox_threshold_lo = (taos_cfgp->prox_threshold_hi *THRES_LO_TO_HI_RATIO);
			}
			#else
			if (taos_cfgp->prox_threshold_lo < ((sat_prox*15)/100)) {				
